...
public class CreateJsonEditor : Editor
{
    ...
    [MenuItem("自定义工具/替换中文文本json")]
    static void RepleceJson()
    {

        JsonData chineseJson = JsonMapper.ToObject(BundleManager.Instance.GetJson("CityChineseConfig"));

        //Debug.Log(chineseJson["万州区"].TryGetString("typeId"));
        foreach (var item in chineseJson.Keys)
        {            
            string key = item;
            if (item[item.Length - 1] == '省' || item[item.Length - 1] == '市' || item[item.Length - 1] == '区' || item[item.Length - 1] == '县')
                key = item.Replace("省", "").Replace("市", "").Replace("区", "").Replace("县", "");
            Debug.Log(key.ToString());
            if (jsonStr.Contains(key))
            {
                string id = chineseJson[item]["id"].ToString();
                jsonStr = jsonStr.Replace(key, id);
            }
        }
        CreateJson();
    }
