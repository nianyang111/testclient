...
public class MiscUtils
{
    ...
    /// <summary>获取OrderedDictionary对应索引的键</summary>
    ...
    public static Texture2D RotateTexture(Texture2D texture, float eulerAngles)
    {
        ...
        Texture2D new_texture = new Texture2D(H, W);
        for (int i = 0; i < H; i++)
        {
            for (int j = 0; j < W; j++)
            {
                new_texture.SetPixel(i, j, texture.GetPixel(j, i));
            }
        }
        new_texture.Apply();
        return new_texture;
    ...
    public static float GetMillionFromByte(float bytes)
    {
    ...
    public static IEnumerator StartGPS(float desiredAccuracyInMeters = 5, float updateDistanceInMeters = 10)
    {
        if (!Input.location.isEnabledByUser)
        {
            UIUtils.Log("isEnabledByUser value is:" + Input.location.isEnabledByUser.ToString() + " Please turn on the GPS");
            yield break;
        }
        else
        {
            ...
            if (maxWait < 1)
            {
                UIUtils.Log("Init GPS service time out");
                yield break;
            }

            if (Input.location.status == LocationServiceStatus.Failed)
            {
                UIUtils.Log("Unable to determine device location");
                yield break;
            }
            else
            {
